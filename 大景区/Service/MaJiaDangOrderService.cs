...
namespace 通用订票.大景区.Service
{
    ...
    public class MaJiaDangOrderService : DefaultOrderService, ITransient
    {
        ...
        public override Task<Core.Entity.Order> CreateOrder(Guid objectId, string name, decimal amount, OrderStatus status = OrderStatus.未付款, string extraInfo = null)
        {
            _log.Log(LogLevel.Information, "大景区订单开始下单");
            return base.CreateOrder(objectId, name + "大景区", amount, status, extraInfo);
        }
