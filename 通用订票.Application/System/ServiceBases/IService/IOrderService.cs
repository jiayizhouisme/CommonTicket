...
{
    public interface IOrderService<T> : IBaseOrderService<T> where T : Core.Entity.Order,new()
    {
        public Task<T> CreateOrder(Guid objectId,string name,decimal amount,OrderStatus status = OrderStatus.未付款);
