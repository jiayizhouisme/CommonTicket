...
namespace 通用订票.RedisMQ
{
    public class OrderCreateSubscribe : IRedisSubscribe
    {
        ...
        {
            var entity = new OnOrderCreateFail() { app = stockret, order = order, count = count, tenantId = tenantId, userId = userId };
            await _eventPublisher.PublishAsync(new OrderCreateFailEvent(entity));
        }
